...
export let innerButtonClassName = '';
let showModal = false;
...
<button
	type="button"
	class="btn-radius btn-mh-2 {buttonClassName === '' ? 'btn-neutral btn' : buttonClassName}"
	on:click="{onClick}"
>
	{#if showModal}
...
>
	...
	<ul class="flex w-full flex-col justify-center">
		...
		<Button
			buttonClassName="{innerButtonClassName} btn-neutral justify-start w-full"
			func="{() => console.log('Cosmostation')}"
		>
		...
		<span class="my-4">wallet connector powered by RIZON</span>
	</ul>
</Modal>
